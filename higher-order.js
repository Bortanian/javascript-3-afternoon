...
//Code Here
let myStrongest = monstersInYourPocket.filter((monster,i) => {
      if(monster.CP > 200){
        return monster;
      }
})
...
let bobsTotal = purchases.filter(purchase => purchase.owner === "Bob").map(order => order = order.price).reduce((total,price) => total + price)
